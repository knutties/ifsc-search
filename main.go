...
func handleSearch(s search.Searcher) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			writeSmithyError(w, http.StatusBadRequest, smithyBadRequest, err.Error())
			return
		}
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			if errors.Is(err, search.ErrNotFound) {
				writeSmithyError(w, http.StatusNotFound, smithyBranchNotFnd, "ifsc code not found")
				return
			}
...
// writeSmithyError mirrors writeError but additionally sets the
// X-Amzn-Errortype header so SDKs generated by smithy-aws-typescript-codegen
// (and other restJson1-aware codegens) can deserialize the response into the
// typed error shape modelled in smithy/models/.
func writeSmithyError(w http.ResponseWriter, status int, smithyType, msg string) {
	w.Header().Set("X-Amzn-Errortype", smithyType)
	writeJSON(w, status, map[string]string{"error": msg})
}

const (
	smithyBadRequest    = "io.knutties.banksearch#BadRequest"
	smithyBranchNotFnd  = "io.knutties.banksearch#BranchNotFound"
)
